/**
 * Commodity decision scorer — the parallel to lib/crypto-scoring.ts for
 * futures (Yahoo quoteType FUTURE, e.g. GC=F gold, CL=F crude). Market-data
 * only, same reasoning as crypto: supply/demand/inventory/geopolitical data
 * has no free numeric feed, so it's covered by the AI insight layer
 * (lib/ai-commodity-research.ts, grounded in news) rather than fabricated
 * here as a fake "supply score".
 *
 * Shares the momentum/relative-strength/risk-adjusted-return/drawdown shape
 * with crypto, but recalibrated: futures trade on an exchange calendar
 * (~252 trading days/year, closed weekends) like equities, NOT 24/7/365
 * like crypto — using 365 here would overstate annualized figures by ~45%.
 * Momentum/drawdown ranges are also narrower than crypto's (commodities
 * are volatile relative to equities, but rarely see crypto's 70%+ swings).
 */
...
function threeMonthReturn(history: HistoryPoint[]): number | null {
  ...
  const back = closes.at(-Math.min(63, closes.length))!; // ~63 trading days = 3 months on an exchange calendar
...
function momentumBucket(history: HistoryPoint[]) {
  ...
  return bucket("Momentum", [
    mk("3-month return", r3m, -25, 25, 13, (v) => `${v >= 0 ? "+" : ""}${v.toFixed(0)}% over 3 months`),
    mk("vs recent high", fromHigh, -45, 0, 12, (v) => `${v.toFixed(0)}% vs high over the fetched window`),
  ]);
}

function relativeStrengthBucket(history: HistoryPoint[], benchmarkHistory: HistoryPoint[] | null) {
  if (!benchmarkHistory || benchmarkHistory.length < 2) {
    return bucket("Relative Strength vs Commodity Index", [
      mk("vs DBC (window return)", null, -20, 20, 25, () => ""),
    ]);
  }
  ...
  return bucket("Relative Strength vs Commodity Index", [
    mk("vs DBC (window return)", relative, -20, 20, 25, (v) => `${v >= 0 ? "+" : ""}${v.toFixed(0)}pp vs the broad commodity index over the fetched window`),
  ]);
}

function riskAdjustedBucket(returns: number[]) {
  // 252 trading days/year — futures close on weekends/holidays like equities.
  ...
  return bucket("Risk-Adjusted Return", [
    mk("Sharpe (annualized)", sharpe, -1, 1.5, 13, (v) => `Sharpe ${v.toFixed(2)}`),
    mk("Sortino (annualized)", sortino, -1, 2, 12, (v) => `Sortino ${v.toFixed(2)}`),
  ]);
}
...
function drawdownBucket(returns: number[]) {
  ...
  return bucket("Drawdown Risk", [
    mk("Max drawdown (window)", dd, -55, -8, 13, (v) => `${v.toFixed(0)}% max drawdown in the fetched window`),
    mk("Annualized volatility", annualVolPct, 60, 15, 12, (v) => `${v.toFixed(0)}% annualized volatility`),
  ]);
}

/**
 * Score a commodity future from its own price history and a broad commodity
 * index ETF's (DBC — the same "de facto benchmark" role BTC plays for
 * crypto, SPY for equities).
 */
export function computeCommodityScore(
  ...
  const relative = relativeStrengthBucket(history, benchmarkHistory);
  ...
  const rationaleParts = [
    `Commodity score ${composite}/100 (momentum, relative strength vs the broad commodity index, risk-adjusted return, drawdown risk), market-data only — supply/demand and geopolitical context is covered by the AI insight below, not this score.`,
  ];
