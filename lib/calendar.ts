/**
 * Calendar — real upcoming earnings/dividend/ex-dividend events for the
 * user's portfolio+watchlist symbols, plus a hand-maintained macro event
 * schedule (dates only — no fabricated forecast/actual figures).
 *
 * Extracted from app/api/calendar/route.ts so server-side callers in the
 * same process (Mission Control) can call getCalendarEvents() directly
 * instead of self-fetching the route over HTTP.
 */
...
function warnIfMacroScheduleRunningOut(windowEndStr: string): void {
  ...
  if (lastDate <= warnThreshold.toISOString().slice(0, 10)) {
    console.warn(
      ...
      `Extend the hardcoded schedule in lib/calendar.ts or macro events will start silently disappearing.`,
    );
...
export async function getCalendarEvents(): Promise<CalendarResponse> {
  ...
  if (symbolMap.size === 0) {
    ...
    return { events, portfolioSymbols, watchlistSymbols, unavailableSymbols: [] };
  }
