...
async function eventsFromAlerts(symbol: string, name: string): Promise<TimelineEvent[]> {
  const alerts = await gatherWatchlistAlerts(undefined, {
    items: [{ symbol, name, addedAt: new Date().toISOString(), targetPrice: null, targetDirection: null, alertPctDrop: null, notes: null, buyTrigger: null, sellTrigger: null, conviction: null, horizon: null, lastReviewedAt: null, lastResearchedAt: null, stage: "surfaced", stageChangedAt: null, source: null, sourceDetail: null }],
  });
