 ...
 *   1. Yahoo Finance trending symbols + per-symbol news (yahoo-finance2)
 *   2. Google News RSS — global + India-specific queries
 *   3. Economic Times RSS feed
 *   4. NSE India corporate announcements (public JSON endpoint)
 *   5. NewsAPI.org — optional, set NEWSAPI_KEY env var for ~100 req/day free
 */
...
/* -------------------------------------------------------------------------- */
/* Source: Yahoo Finance trending + news                                       */
/* -------------------------------------------------------------------------- */
...
async function fetchNseAnnouncements(limit = 20): Promise<NewsItem[]> {
  try {
    // NSE requires a session; fetch the homepage first to get cookies.
