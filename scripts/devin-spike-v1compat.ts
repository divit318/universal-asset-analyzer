...
async function oneRun(run: number, prompt: string, schema: Record<string, unknown>): Promise<RunResult> {
  ...
  const body: Record<string, unknown> = {
    ...
    // The "UAA Analyst" playbook forbids questions and mandates
    // provide_structured_output — the contract v1 cannot express via
    // structured_output_required. Attached when synced to this key's scope.
  ...
  for (;;) {
    ...
    if (phase === "waiting_for_user" && s.structured_output == null) {
