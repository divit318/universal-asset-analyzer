...
export function ConvictionBreakdown({ score, loading, risks, onViewRisks }: Props) {
  ...
  // Not one fundamental factor resolved. Every bucket below is mk()'s
  // half-credit padding, which sums to a confident-looking ~51/100 — so the
  // breakdown is withheld rather than dressed up as analysis. This is what an
  // index or an unmapped quoteType reaching the equity path looks like.
  if (score.fundamentalCoverage && score.fundamentalCoverage.available === 0) {
    return (
      <div className="flex flex-col gap-2 rounded-xl border border-border bg-surface px-5 py-8 text-center">
        <p className="text-sm font-medium text-foreground">
          No fundamental data for this instrument
        </p>
        <p className="mx-auto max-w-md text-xs leading-5 text-muted">
          Valuation, quality, growth and balance-sheet scoring need company financials,
          and the data provider publishes none for this symbol. A conviction score is
          deliberately not shown — it would be built entirely from defaults.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* ── Top row: how the score was reached. The verdict itself (badge,
             composite, data confidence) is NOT repeated here — the hero above
             owns the call; this tab owns the evidence behind it. ── */}
      {(score.rationale || (risks?.length ?? 0) > 0) && (
      <div className="card-lift flex flex-wrap items-center gap-x-4 gap-y-2 rounded-xl border border-border bg-surface p-4">
        {score.rationale && (
          <p className="min-w-0 flex-1 text-[11px] leading-5 text-muted">{score.rationale}</p>
        )}
        {risks && risks.length > 0 && (() => {
          ...
          return (
            <button
              ...
              className={`inline-flex shrink-0 items-center rounded-lg border px-3 py-1 text-xs font-semibold tracking-wide ${chip.cls} ${onViewRisks ? "cursor-pointer hover:opacity-80" : ""}`}
              title={worst.reason}
            >
              {chip.label}
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-3">
          ...
          <div className="h-px flex-1 bg-border" />
          <span className="text-[10px] text-muted/50">Weakest first · all subscores /100</span>
        </div>
        ...
        {/* A grid, not a stack.
            ...
            show them together. */}
        {/* Weakest first: six near-equal cards in declaration order read as
            "everything is fine" even when one dimension is genuinely poor.
            Triage order puts what needs attention where reading starts. */}
