...
const STARTER_QUESTIONS = [
  "Is this stock cheap relative to its growth?",
  "How does insider activity affect the thesis?",
  "What are the biggest red flags in the data?",
  "Compare its margins to the sector median.",
  "Is analyst consensus bullish or bearish and why?",
];

export function AiDeepPanel({
  quote,
  fundamentals,
  peers,
}: {
  ...
  async function runAnalysis() {
    ...
    try {
      const res = await fetch("/api/ai/deep", {
        ...
        body: JSON.stringify({ mode: "analysis", quote, fundamentals, peers }),
      });
  ...
  return (
    <section className="flex flex-col gap-4 rounded-xl border border-border bg-surface p-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-base font-semibold">AI Equity Research</h2>
          <p className="text-xs text-muted">
            Structured research note grounded in scoring engine, analyst consensus, insider activity, peer comparison &amp; SEC filings
          </p>
        </div>
      ...
      {mode === "idle" && !loading ? (
        <p className="text-sm text-muted">
          Run a deep AI analysis that synthesises the scoring engine output, analyst consensus,
          insider activity, peer comparison, and SEC statements into a structured research note.
          Or use Ask AI for freeform questions.
        </p>
      ) : null}
      ...
      {mode === "idle" && loading ? (
        <div className="flex items-center gap-2 text-sm text-muted">
          ...
          Running deep analysis across all data sources…
        </div>
      ) : null}
    </section>
  );
